...
/**
 * Animate a progress bar update
 * 
 * @param  {Number} value The new value to use.
 * @return {Promise}      Resolved on animation completion.
 */
PercentBarSpinner.prototype.tween = function(value) {
  var d = q.defer(),
  s = this,
  c = 0,
  diff = (value - this.current);
  async.whilst(
    function condition() {
      return c < diff;
    },
  ...
  function tweenUpdate(s) {
    var bar = s.barstr();
    var val = s.percent();
    s.update(sprintf(' %4d%%', val) + ' [' + s.barstr() + '] ');
  }
  return d.promise;
};
...
PercentBarSpinner.prototype.width = function() {
  return process.stdout.getWindowSize()[0] - 13;
};
